defmodule Plug.TdHypermedia do
  ...
  def put_hypermedia(conn, helper, assigns) do
    resource_type = Keyword.get(assigns, :resource_type, [])
    {assign_key, resources} = find_resources(assigns)

    rendered = render_hypermedia(helper, conn, resources, resource_type, assign_key)
  ...
  defp render_hypermedia(helper, conn, resources, resource_type, assign_key) do
    helper
    |> build_hypermedia(conn, resources, resource_type)
    |> View.render_view(assign_key)
  end

  defp build_hypermedia(helper, conn, resources, resource_type) do
    Hypermedia.build(helper, conn, resources, resource_type)
  end
